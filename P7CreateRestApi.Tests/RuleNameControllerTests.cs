...
public class RuleNameControllerTests
{
    ...
    private static RuleNameController CreateController(LocalDbContext context)
    {
        RuleNameRepository repository = new RuleNameRepository(context);
        RuleNameController controller = new RuleNameController(
            repository, NullLogger<RuleNameController>.Instance);
    ...
    public async Task GetById_RenvoieNotFound_SiInexistant()
    {
        ...
        RuleNameController controller = CreateController(context);
    ...
    {
        using LocalDbContext context = CreateContext();
        RuleName created = await new RuleNameRepository(context).Add(new RuleName { Name = "R1" });
        RuleNameController controller = CreateController(context);

        IActionResult result = await controller.GetById(created.Id);

        OkObjectResult ok = Assert.IsType<OkObjectResult>(result);
        Assert.IsType<RuleName>(ok.Value);
    }
    ...
    public async Task Create_Renvoie201Created()
    {
        ...
        IActionResult result = await controller.Create(new RuleName { Name = "R1" });

        CreatedAtActionResult created = Assert.IsType<CreatedAtActionResult>(result);
        RuleName returned = Assert.IsType<RuleName>(created.Value);
    ...
    public async Task Update_RenvoieNotFound_SiInexistant()
    {
        ...
        IActionResult result = await controller.Update(999, new RuleName { Name = "R1" });
